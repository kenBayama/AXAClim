...
from dependencies.utils import read_csv, load_data, read_parquet_and_createStore
...
def transform_dataframe (input_data):
    ...
    clean the company client portofolio data

    Transformations : 

        AGGREGATE the total number of client (00021_NUMCLIA) per portofolio (NMPTF)
        AGGREGATE the total number of contracts (00004_NUMCLE) per portofolio (NMPTF)
        AGGREGATE number of contracts per portofolio (NMPTF) and per term (01255_MOISAN)
        PIVOT the term (01255_MOISAN) for each portofolio (NMPTF) and store the number of contract
            per portofolio (NMPTF) and per term (01255_MOISAN) in the newly created columns(ECH)
    ...
    total_clients = input_data.groupBy("NMPTF").agg(countDistinct("00021_NUMCLIA").alias("TOTAL_CLIENTS"))
    total_contracts = input_data.groupBy("NMPTF").agg(countDistinct("00004_NUMCLE").alias("TOTAL_CONTRATS"))

    # AGGREGATE the number of contracts per portofolio and per term
    echeance_first_step = input_data.groupBy("NMPTF","01255_MOISAN").agg(countDistinct("00004_NUMCLE").alias("ECH"))
    
    # PIVOT the term for each portofolio
    echeance_last_step = echeance_first_step.groupBy("NMPTF").pivot("01255_MOISAN").agg(first("ECH").alias("_ECH"))

    # RENAME the newly created column
    pivoted_new_col = echeance_last_step.columns
    pivoted_new_col.remove("NMPTF") 
    echeance_final =( reduce((lambda df, col_name: df.withColumnRenamed(col_name,"ECH_"+col_name)),
        pivoted_new_col, 
        echeance_last_step
    ))

    # JOIN all the resulting dataFrames   
    transformed_df_final = total_clients.join(total_contracts,on="NMPTF",how="left").join(echeance_final, on="NMPTF",how="left")
...
def main() :
    ...
    input_data = read_parquet_and_createStore(spark, path, store_name, usefull_col)
